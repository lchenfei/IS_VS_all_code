#' OptiTreeStrata Method - Example 2
#' 
#' Wrapper for OptiTreeStrata stratified sampling method (D=5, correlated).
#' Based on case='4' in the original OptiTreeStrata code.
#'
...
#' @param n_pilot Pilot sample size (default: 240)
...
run_optitree <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    case <<- '4'  # Ex2 corresponds to case='4' in OptiTreeStrata (correlated)
    
    # Source OptiTreeStrata functions
    source(file.path(ROOT, "AMISE_univariate.R"))
    source(file.path(ROOT, "function_OptiTreeStrata_dependent.R"))
    
    set.seed(seed_base + 50 + j)
    print(paste0('OptiTreeStrata Ex2, experiment:', j))
    
    # Parameters for Example 2 (case='4', correlated)
    p <<- 5
    l <<- 24.2
    max_x <<- 5
    min_x <<- -5
