#' IS-Pareto Method for Importance Sampling Variable Selection - Example 3
#' 
#' Uses Pareto-k diagnostic for subset selection with bivariate kernel density estimation.
#'
#' @param n_reps Number of independent replications (default: 25)
#' @param ncores Number of parallel cores (default: 25)
#' @param seed_base Base seed for reproducibility (default: 1235)
...
run_is_pareto <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    for (simu in 1:n_iter) {
      ...
      # Order subsets from the smallest CE to the largest
      ...
      index_pareto_list <- sapply(h_s_all, function(x) setequal(x, subset))
      ...
      weight_ce_all <- c(weight_ce_all, list(weight_s))
