#' IS-CE Method for Importance Sampling Variable Selection - Example 4
#' 
#' Uses greedy Cross-Entropy based subset selection for D=50.
#' Uses find_ce_order_greedy (no Pareto-k filtering).
#'
...
run_is_ce <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    for (simu in 1:n_iter) {
      ...
      # Greedy CE ordering (no Pareto-k filtering)
      h_s_ce <- find_ce_order_greedy(Nm, sample_X_all, sample_Z_all, h_new_mat, combo, weight, S_all)
