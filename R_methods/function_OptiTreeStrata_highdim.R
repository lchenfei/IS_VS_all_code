...
x_dist <- function(x){
  dist = switch(case,
                ...
                '2' = dnorm(x,0, 1),
                '3' = dnorm(x,0, 1),
                'Multi_modal' = dunif(x,min_x, max_x),
                'Fluc_mean' = dunif(x,min_x, max_x),
                'Fluc_variance' = dnorm(x,0, 1),
                'EggCrate' = dunif(x,min_x, max_x),
                'Mishra05' = dunif(x,min_x, max_x),
                'high_dimension' = dnorm(x,0, 1),
                'Gamma_1' = dgamma(x,5,5),
                'Gamma_2' = dgamma(x,5,5),
                'Gamma_3' = dgamma(x,5,5),
                'Uniform_1' = dunif(x,min_x, max_x),
                'Uniform_2' = dunif(x,min_x, max_x),
                'Uniform_3' = dunif(x,min_x, max_x),
                't_1' = dt(x,4),
                't_2' = dt(x,4),
                't_3' = dt(x,4),
...
Make_Y <- function(X){
  Y = switch(case,
             ...
             'Multi_modal' = mu_condi_modal(X) + rnorm(nrow(X),sd = 0.5),
             'Fluc_mean' = Fluc_mean(X) + rnorm(nrow(X),sd = 0.3),
             'Fluc_variance' = rnorm(nrow(X), sd = abs(X[,1])),
             'EggCrate' = EggCrate(X) + rnorm(nrow(X),sd = 0.3),
             'Mishra05' = Mishra05(X) + rnorm(nrow(X),sd = 0.3),
             ...
             "Uniform_1" = rnorm(nrow(X), mean = mu_condi1(X),sd = 1),
             "Uniform_2" = rnorm(nrow(X), mean = mu_condi2(X),sd = 1),
             "Uniform_3" = rnorm(nrow(X), mean = mu_condi3(X),sd = 1),
             "t_1" = rnorm(nrow(X), mean = mu_condi1(X),sd = 1),
             "t_2" = rnorm(nrow(X), mean = mu_condi2(X),sd = 1),
             "t_3" = rnorm(nrow(X), mean = mu_condi3(X),sd = 1),
...
# multi-modal
mu_condi_modal = function(X){
  X1 = X[,1];  X2 = X[,2];  X3 = X[,3];
  m = 3 - sin(X1) - sin(2 * X1) - sin(X2)^2 - sin(X3)^2 - exp(-X1^2-X2^2-X3^2)
  return(m)
}

# Fluc_mean
Fluc_mean  = function(x){
  if(is.data.frame(x) == TRUE){
    x = matrix(x[,1])
    (0.5* sin(x) - 0.5*sin(-2))*(x < -2.5) + (3*sin(2 * x)  - 3*sin(4))*(x>3) 
  }else{
    (0.5* sin(x) - 0.5*sin(-2))*(x < -2.5) + (3*sin(2 * x)  - 3*sin(4))*(x>3) 
  }
}
...
# Fluc_variance
Fluc_variance  = function(x){
  X1 = X[,1];  X2 = X[,2];  X3 = X[,3];
  return(X1^2 + 0.1*X2 + 0.1*X3)
}

Fluc_variance_error = function(x){
  if(is.data.frame(x) == TRUE){
    x = matrix(x[,1])
    (0.1 + 0.5*abs(x))* (abs(x) < 2.5) + (abs(x))* (abs(x) >= 2.5)
  }else{
    (0.1 + 0.5*abs(x))* (abs(x) < 2.5) + (abs(x))* (abs(x) >= 2.5)
  }
}

## Multi_modal distribution [-3.5,3.5]
EggCrate = function(X){
  X1 = X[,1];  X2 = X[,2];  X3 = X[,3];
  return(X1^2 + 0.2*X2^2 + 25 *(sin(X1)^2 + 0.2 * sin(X2)^2) + 0.1*X3)
}

## x1 in [-2,2]
Mishra05 = function(X){
  X1 = X[,1];  X2 = X[,2]; 
  return((sin((cos(X1) + 0.5 *cos(X2))^2)^2 + cos((sin(X1) + 0.5 * sin(X2))^2)^2 + X1)^2 + 0.01*(X1 + 0.5*X2))
}
...
Draw_Pilot_sample_OA_LHS = function(n_allocation, p, min_x, max_x){
  ...
  # for (j in col_shuff){
  #   if((case == 'Multi_modal')| (case == 'Fluc_mean') | (case == 'Fluc_variance')|(case == 'Uniform_1')|(case == 'Uniform_2')|(case == 'Uniform_3')){
  #     X[,j] = qunif(D[,j]/nrow(D) - runif(nrow(D))/nrow(D),min_x,max_x)
  #   }else if((case == 'Gamma_1')|(case == 'Gamma_2')|(case == 'Gamma_3')){
  #     X[,j] = qgamma(D[,j]/nrow(D) - runif(nrow(D))/nrow(D),5,5)
  #   }else if((case == 't_1')|(case == 't_2')|(case == 't_3')){ # t
  #     X[,j] = qt(D[,j]/nrow(D) - runif(nrow(D))/nrow(D),4)
  #   }else{
  #     X[,j] = qnorm(D[,j]/nrow(D) - runif(nrow(D))/nrow(D))
  #   }
  # }
  Y = Make_Y(X)
  sample = data.frame(X=X, Y=Y)
...
Tree_plot = function(Best_strata){
  ### adjust size 
  size = 8; font = 0.8; symbol = 15; background = 'yellow'
  
  Tree = Best_strata$Full_Strata
  depth = length(Tree); coordinate =  vector(mode = "list", length = depth)
  for(d in 0:(depth-2)){
    if(d==0){
    ...
    if(d == 0){
      ...
      plot(1:nrow(node),-node[,1],pch = symbol ,col='red', cex = terminal_size, ylim = c(-depth-0.5,-0.5), xlim = c(0,nrow(node)+0.5),
      ...
      for(i in 1:nrow(node)){
        ...
        n = dim(temp)[1]
        text(i,-node[i,1],paste('X',v,'\n',interval,'\n','n=',n,'\n',Best_strata$n_optimal[i]),cex=font)
      }
    }
    ...
    for(i in 1:nrow(draw_next)){
      ...
      segments(coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$x,-coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$y
               ,coordinate[[depth-d]][[node[sameparent[1],2]]]$x, -coordinate[[depth-d]][[node[sameparent[1],2]]]$y,col=background)
      segments(coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$x,-coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$y
               ,coordinate[[depth-d]][[node[sameparent[2],2]]]$x, -coordinate[[depth-d]][[node[sameparent[2],2]]]$y,col=background)
      ...
      if(draw_next[1,1] == 1 & draw_next[1,2] == 1){
        temp = Tree[[1]]
        n = dim(temp)[1]
        text(coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$x,-coordinate[[draw_next[i,1]]][[draw_next[i,2]]]$y,
             paste('Root','\n','n=',n),cex=font)
      }else{
...
Simulation_MSS = function(Pilot_sample,Best_strata,l,n_simulation,nj_min,m,I,l_RR, r, Num_iter){
  ...
  if(Num_iter == 1){
    ...
    Ehat_MSS = mean(Ehat_MSS_inner); Ehat_MSS_obj = mean(Ehat_MSS_inner_obj)
    return(list('Best_strata' = Best_strata,'Ehat_MSS' = Ehat_MSS,'Tree'=Tree,'Tree_save'=Tree_save,'Ehat_MSS_obj' = Ehat_MSS_obj)) 
  }
}
...
Simulation_MEqui = function(Pilot_sample,I,p,l,n_simulation,min_x, max_x,nj_min){
  ...
  for(i in 1:length(Tree[[depth]])){
    ...
    temp_Pilot = Tree[[1]][paste('X.',substring(colnames(temp)[dim(temp)[2]],9,9),sep='')][,1] #select variable
    n_num <- nrow(temp)
    mu_equi[i] = mean(temp$Z, na.rm = T)
    s_equi[i] = mean((temp$Z)^2, na.rm = T)
