#' WAMK-SIS Method for Importance Sampling - Example 3
#' 
#' Uses Weighted Additive Multi-Kernel approach with all 45 kernel subsets (D=10).
#'
...
run_wamk_sis <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    for (simu in 1:n_iter) {
      ...
      S_all <- matrix(S_all, nrow = length(S_all) / 45)  # 45 = C(10,2)
    ...
    #     index = index, weight_s_all = weight_s_all, bandwidth = bandwidth)
    mean(est_vec)
  }
