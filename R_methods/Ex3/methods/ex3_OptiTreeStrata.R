#' OptiTreeStrata Method - Example 3
#' 
#' Wrapper for OptiTreeStrata stratified sampling method (D=10, independent).
#' Based on case='3' in the original OptiTreeStrata code.
#'
...
#' @param n_pilot Pilot sample size (default: 340)
...
run_optitree <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    case <<- '3'  # Ex3 corresponds to case='3' in OptiTreeStrata
    ...
    print(paste0('OptiTreeStrata Ex3, experiment:', j))
    
    # Parameters for Example 3 (case='3')
    p <<- 10
    l <<- 18.74
    max_x <<- 4  # Note: max_x=4 for case='3'
    min_x <<- -4
