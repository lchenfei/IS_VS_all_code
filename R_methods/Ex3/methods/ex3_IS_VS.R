...
#' For D=10 (high-dimensional), uses sampling_ce_order_greedy_2 for efficient subset search.
#'
...
run_is_vs <- function(n_reps = 25, 
  ...
  POE_est <- foreach(j = 1:n_reps, 
    ...
    for (simu in 1:n_iter) {
      ...
      # Greedy CE ordering with Pareto-k filtering (for D=10)
    ...
    #     index_all = index_all, bandwidth = bandwidth)
    mean(est_vec)
  }
