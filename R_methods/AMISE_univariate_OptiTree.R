...
integrand_a = function(x_vec,X,Z,h){
  int_a = (S_d2(x_vec,X,Z,h)/2+S_d1(x_vec,X,Z,h)*S2_d1(x_vec,X,Z,h)/S2(x_vec,X,Z,h))^2 #Eq(13)
  int_a[is.na(int_a)] = 0
  int_a[int_a==Inf] = 0
  return(int_a)
}
...
get_a = function(X,Z,h,left=-Inf,right=Inf){
  ...
  a = hcubature(function(x) integrand_a(x,X,Z,h), lowerLimit = left, upperLimit = right)
  a = a$integral
  return(a)
}
...
integrand_b = function(x_vec,X,Z,h){
  ...
  if(Interest == 'Failure_prob'){
    int_b = S(x_vec,X,Z,h)*(1-S(x_vec,X,Z,h))/S2(x_vec,X,Z,h)*sqrt(2*pi)*n*h #Eq 16
  }else if(Interest == 'Shortfall'){
    sigma_square_hat = var(Z - S(x_vec,X,Z,h))
    int_b = sigma_square_hat/S2(x_vec,X,Z,h)*sqrt(2*pi)*n*h
  }
