...
x2given1_dist = function(x1,x2){
  p = dnorm(x2, x1, 1)
  return(p)
}

x12_dist = function(x1,x2){
  p = dnorm(x2, x1, 1) * x1_dist(x1)
  return(p)
}

x3given1_dist = function(x1,x3){
  p = dnorm(x3, x1, 1)
  return(p)
}

x13_dist = function(x1,x3){
  p = dnorm(x3, x1, 1) * x1_dist(x1)
  return(p)
}
...
x2_dist = function(x2){
  if(length(x2)>=2){
    p = sapply(x2,function(i){integrate(function(x1) x12_dist(x1,i),-5,5)$value})
    return( p)
  }else{
    p = integrate(function(x1) {x12_dist(x1,x2)},-5,5)$value
    return( p)
  }
...
x3_dist = function(x3){
  if(length(x3)>=2){
    p = sapply(x3,function(i){integrate(function(x1) x13_dist(x1,i),-5,5)$value})
    return( p)
  }else{
    p = integrate(function(x1) {x13_dist(x1,x3)},-5,5)$value
    return( p)
  }
  
}

X_joint <- function(Range){
  #Range should be vector and it is (min, max, min, max...)
  non_zero = unique(ceiling((which(!is.na(Range))) / 2))
  if(sum(is.na(Range))>=1){
    if(length(non_zero) == 1){
      x_dist = function(x){
        X = switch(non_zero,'1' = x1_dist(x),'2' = x2_dist(x), '3' = x3_dist(x))
        return(X)
      }
      return(integrate(x_dist,Range[!is.na(Range)][1],Range[!is.na(Range)][2])$value)
    }else if(length(non_zero) == 2){
      if(paste(non_zero,collapse = '') == '23'){
        maximum_range = c(-5, 5, -5,5,-5,5)
        Range[which(is.na(Range))] = maximum_range[which(is.na(Range))]
        return(integrate(Vectorize(function(x3) { 
          integrate(Vectorize(function(x2) { 
            integrate(function(x1) { x1_dist(x1) * x2given1_dist(x1,x2) * x3given1_dist(x1,x3) },Range[1], Range[2])$value }), Range[3], Range[4])$value }), Range[5], Range[6])$value)
      }else{
        x_dist = function(x1,x2){
          X = switch(paste(non_zero,collapse = ''),'12' = x12_dist(x1,x2),'13' = x13_dist(x1,x2))
          return(X)
        }
        return(integrate(Vectorize(function(x2) { 
          integrate(function(x1) { x_dist(x1,x2) },Range[!is.na(Range)][1], Range[!is.na(Range)][2])$value }), 
          Range[!is.na(Range)][3], Range[!is.na(Range)][4])$value)
      }
    }else{
      maximum_range = c(-5, 5, -5,5,-5,5)
      Range[which(is.na(Range))] = maximum_range[which(is.na(Range))]
...
Draw_Pilot_sample_OA_LHS = function(n_allocation, p, min_x, max_x){
  prime_number = switch(as.character(n_allocation),'100' = 5, '170' = 13,'200' = 7, '240' = 11, '340' = 13,'500' = 11, '1000' = 11)
  lambda = switch(as.character(n_allocation),'100' = 4,'170' = 1 ,'200' = 4, '240' = 2, '340' = 2, '500' = 4, '1000' = 8)
...
## Calculate each stratum's values for Indicator
Calculate_strata_value_ind = function(temp,Z,h_hat,a,input){
  X = temp[,input]
  ...
  x_dist = function(x){
    X = switch(input,'1' = x1_dist(x),'2' = x2_dist(x), '3' = x3_dist(x), '4' = x4_dist(x), '5' = x5_dist(x))
    return(X)
  }
  if(input == 4 | input == 5){
    ...
    for(i in 1:I){
      ...
      s_strata[i] = mu_strata[i] # For estimating failure probability
      sd_strata[i] = sd_strata[i] = sqrt(s_strata[i] - mu_strata[i]^2)
    }
    k = 1 / sd_strata
    k[k == Inf | k == - Inf] = 100000 #for preventing errors
    return(data.frame(p_strata,mu_strata, s_strata, sd_strata,k))
  }else{
    # figure out how many split occured
    split_num = dim(temp)[2] - which(colnames(temp) == 'Z')
    
    ## If split occurs, we need to figure out conditional probability
    ## Otherwise, we can get strata value directly
    if(split_num == 0){
      Expectation_stratum = function(x){hx(x) * x_dist(x)}
      p_strata = c(); mu_strata = c(); sd_strata = c();s_strata = c();
      strata_break = cut(X,a)
      ...
      ## We need to figure out conditional probability (once split occur)  
      Range = rep(NA,2 * 3) # for saving range of each variable
      for(ii in 1:split_num){  #number of split
        ...
        split_variable = as.numeric(substring(colnames(temp)[ dim(temp)[2]-split_num+ii],9,9))
        if(split_variable <=3){
          Range[split_variable*2 - 1] = lower; Range[split_variable*2] = upper
        }
      }
      ## Define conditional distribution by numerical integral
      denominator = X_joint(Range)
      given = unique(ceiling((which(!is.na(Range))) / 2))
      variable = unique(c(given, input))
      # To calculate probability
      maximum_range = c(-5, 5,-5,5,-5,5)
      Range[which(is.na(Range))] = maximum_range[which(is.na(Range))]
      
      
      # Decide conditional density according to combinations of variables
      if(length(variable) == 1){
        ...
        # When the stratum is divided by two input variables
        input = as.character(input)
        condi_density = function(x){
          X = switch(paste(variable,collapse = ''),
                     '12' = switch(input,'1' = integrate(Vectorize(function(x) { 
                       integrate(function(x2) { x12_dist(x,x2) },Range[3], Range[4])$value}), x, x + 1e-6)$value / 10^(-6)/ denominator,
                       '2' = integrate(Vectorize(function(x) { 
                         integrate(function(x1) { x12_dist(x1,x) },Range[1], Range[2])$value}), x, x + 1e-6)$value/ 10^(-6) / denominator), 
                     '13' = switch(input,'1' = integrate(Vectorize(function(x) { 
                       integrate(function(x3) { x13_dist(x,x3) },Range[5], Range[6])$value}), x, x + 1e-6)$value/ 10^(-6)/ denominator,
                       '3' = integrate(Vectorize(function(x) { 
                         integrate(function(x1) { x13_dist(x1,x) },Range[1], Range[2])$value}), x, x + 1e-6)$value/ 10^(-6)/ denominator),
                     '23' = switch(input,'2' = integrate(Vectorize(function(x) { 
                       integrate(Vectorize(function(x3) { 
                         integrate(function(x1) { x1_dist(x1) * x2given1_dist(x1,x) * x3given1_dist(x1,x3) },Range[1], Range[2])$value }), Range[5], Range[6])$value }), x, x + 1e-6)$value/ 10^(-6)/ denominator,
                       '3' = integrate(Vectorize(function(x) { 
                         integrate(Vectorize(function(x2) { 
                           integrate(function(x1) { x1_dist(x1) * x2given1_dist(x1,x2) * x3given1_dist(x1,x) },Range[1], Range[2])$value }), Range[3], Range[4])$value }), x, x + 1e-6)$value/ 10^(-6)/ denominator))
          return(X)
        }
        Expectation_stratum = function(x){hx(x) * condi_density(x)}
        p_strata = c(); mu_strata = c(); sd_strata = c();s_strata = c();
        strata_break = cut(X,a)
        strata = split(X,strata_break)
        I = length(a)-1
        ...
        for(i in 1:I){
          p_strata[i]  = as.numeric(integrate(Vectorize(condi_density),a[i],a[i+1])[1]) #find Pi from condi_density
          mu_strata[i] =  as.numeric(integrate(Vectorize(Expectation_stratum),a[i],a[i+1])[1])
        ...
        # When the stratum is divided by three input variables
        input = as.character(input)
        condi_density = function(x){
          X = switch(input, '1'= integrate(Vectorize(function(x) { 
            integrate(Vectorize(function(x3) { 
              integrate(function(x2) { x1_dist(x) * x2given1_dist(x,x2) * x3given1_dist(x,x3) },Range[3], Range[4])$value }), Range[5], Range[6])$value }), x, x + 1e-6)$value/ 10^(-6)/ denominator,
            '2' = integrate(Vectorize(function(x) { 
              integrate(Vectorize(function(x3) { 
                integrate(function(x1) { x1_dist(x1) * x2given1_dist(x1,x) * x3given1_dist(x1,x3) },Range[1], Range[2])$value }), Range[5], Range[6])$value }), x, x + 1e-6)$value/ 10^(-6)/ denominator,
            '3' = integrate(Vectorize(function(x) { 
...
# 
#   for(i in 1:50){
#     strata_value = Calculate_strata_value_ind(X,Z,h_hat,a)
...
Find_a = function(temp,X,Z,h_hat,a,input){
  a_list = list(); a_list[[1]] = a; obj_list = list()
  x_dist = function(x){
    X = switch(input,'1' = x1_dist(x),'2' = x2_dist(x), '3' = x3_dist(x),'4' = x4_dist(x), '5' = x5_dist(x))
    return(X)
  ...
  for(i in 1:50){
    strata_value = Calculate_strata_value_ind(temp,Z,h_hat,a,input)
    p_strata = strata_value[,1];  mu_strata = strata_value[,2]
    s_strata = strata_value[,3];  sd_strata = strata_value[,4]
    k = strata_value[,5];    I = length(a)-1
    obj_list[[i]] = sum(p_strata * sd_strata) # checking if the algorithm is working correctly
    ...
    a_list[[i+1]] = a
    if(sum(abs(a_list[[i+1]] - a_list[[i]])) < 0.001){
      return(list(p_strata,mu_strata,s_strata,sd_strata,k,a))
      break
    }
  }
  return(list(p_strata,mu_strata,s_strata,sd_strata,k,a))
...
Search_variable = function(temp,Z,I,p){
  ...
  for(v in 1:p){
    ...
    #Set initial breaking points
    #checking the variable already split
    min_x = switch(v,'1' = -5,'2' = -5,'3' = -5, '4' = -5,'5' = -5)
    max_x = switch(v,'1' = 5,'2' = 5,'3' = 5, '4' = 5, '5' = 5)
    if(sum(paste('split_',colnames(temp)[v],sep='') == colnames(temp))>0){
      variable = max(which((paste('split_',colnames(temp)[v],sep='') == colnames(temp))==TRUE))
      interval = as.character(temp[,variable][1])
      lower = as.numeric(substring(interval,2,unlist(gregexpr(',',interval))-1))
      ...
      a = seq(min_x,max_x,length.out = I + 1); a_list[[1]] = a #equi-distant
    }
    
    ## finding the optimal width
    result = Find_a(temp,temp[,v],temp$Z,h_hat[v],a,v) # use h_hat from entire input space
    obj_value[v] = sum(result[[1]] * result[[4]])
    p_strata[[v]] = result[[1]]; mu_strata[[v]] = result[[2]]; sd_strata[[v]] = result[[4]];
    a_variable[[v]] = sort(result[[6]])
    summary_result = list('obj_value' = obj_value, 'breaking_points' = a_variable, 
...
Calculate_Mstrata_value_ind = function(Tree,depth,l){
  p_strata = c(); mu_strata = c(); sd_strata = c();s_strata = c();
  for(i in 1:length(Tree[[depth]])){
    ...
    if(is.null(temp)){
      ...
      # print(head(temp))
      # print(paste('X.',substring(colnames(temp)[dim(temp)[2]],9,9),sep=''))
      ...
      Range = rep(NA,2 * 5) # for saving range of each variable
      for(ii in 1:split_num){  #number of split
        interval = as.character(temp[, dim(temp)[2]-split_num+ii][1])
        lower = as.numeric(substring(interval,2,unlist(gregexpr(',',interval))-1))
        upper = as.numeric(substring(interval,unlist(gregexpr(',',interval))+1,unlist(gregexpr(']',interval))-1))
        input = as.numeric(substring(colnames(temp)[ dim(temp)[2]-split_num+ii],9,9))
        Range[input*2 - 1] = lower; Range[input*2] = upper
      }
      non_zero = unique(ceiling((which(!is.na(Range))) / 2))
      if(4 %in% non_zero){
        p_strata[i] = X_joint(Range[1:6])  * integrate(x4_dist,Range[7],Range[8])$value
      }else if(5 %in% non_zero){
        p_strata[i] = X_joint(Range[1:6])  * integrate(x4_dist,Range[9],Range[10])$value
      }else{
        p_strata[i] = X_joint(Range[1:6])
      }
      mu_strata[i] = mean(sum(temp$Y > l)/length(temp$Y)) 
      s_strata[i] = mu_strata[i]
...
Tree_stratification = function(Data,n_simulation,nj_min,l,I,m,l_RR){
  ...
  for(i in 1:nrow(node)){
    Strata_result = Calculate_Mstrata_value_ind(Strata,node[i,1],l)
  ...
  while((RR >= l_RR) & (split_num <= 5)){ # while((RR >= l_RR) & (split_num <= floor(1 / l_RR - 1)))
    k = k + 1
    ...
    for(ii in 1:nrow(node)){
      ...
      for(i in i_range){
        temp_result = Calculate_Mstrata_value_ind(Strata,node[i,1],l)
      ...
      temp_strata_value = Calculate_Mstrata_value_ind(temp_split,node[ii,1],l)
  ...
  for(S_alpha in unique(Strata_alpha)){
    ...
    if( abs(sum(n_optimal[lack_ind] - nj_min)) >= 1){
      ...
      if(sum(n_optimal) !=  n_simulation / m ){
...
Draw_Sample_optimal_allocation = function(Best_strata,n_optimal,l){
    ...
    if(draw_num >= 10000){
      for(i in 1:I){
        if(num_sample[i] < n_optimal[i]){
          ...
          X.1 <- runif(5*n_allocation,-5,5)
          X.2 <- runif(5*n_allocation,-5,5)
          X.3 <- runif(5*n_allocation,-5,5)
          X.4 <- runif(5*n_allocation,-5,5)
          X.5 <- runif(5*n_allocation,-5,5)
      ...
      for(i in 1:I){
        if(num_sample[i] < n_optimal[i]){
          ...
          X.1 <- rnorm(5 * n_allocation, 0, 1) # rrayleigh(5*n_allocation, scale = sqrt(2/pi)*10)
          X.2 <- rnorm(5 * n_allocation, X.1, 1)
          X.3 <- rnorm(5 * n_allocation, X.1, 1)
          X.4 <- rnorm(5 * n_allocation, 0, 1)
          X.5 <- rnorm(5 * n_allocation, 0, 1)
          X <- cbind(X.1,X.2,X.3,X.4,X.5)
  ...
  for(i in 1:I){
    ...
    Sample[[i]]$Z = ifelse(Sample[[i]]$Y > l,1,0)
  }      
  return(Sample)
}
