function [maximum_loads] = NREL_simulator_local_tmp_if_Unix_Extension(run_ind, wind_speed,TI,PLExp,VFlowAng,Z0,NewRandomSeed, simNum, isUnix)
...
% Run on Simulator2 folder (not Simulator folder) in case file names are overlapped. 
...
elseif isUnix ==0   %If Windows
     Simulator_Folder = horzcat(pwd,'\Imple\Simulator', num2str(simNum),'\'); %'C:\Research\Implementation\Simulator4\';     
    Copy_command ='copy ';
    Delete_command = 'del ';
...
if size(Fast_output,2) ~= 13200 %if the output is not fully generated for any reason, run the executable on auxiliary simulator folder.
    ...
    maximum_loads = Aux_simulator_tmp_if_Unix(run_ind, wind_speed,TI,PLExp,NewRandomSeed,isUnix);   %Run on auxiliary simulator not the original one (running on original one repeated the same problem.)
