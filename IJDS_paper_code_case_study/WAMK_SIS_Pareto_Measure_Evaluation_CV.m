function [ final_est, est_vec, bandwidth_cell, weight_cell, weight_cellv, X,Y, v_cell, list_cell, evaluation_cell] = WAMK_SIS_Pareto_Measure_Evaluation_CV(samples, responses, target_quantile,N_T, LoadType,exp_ind)
%% Setup 
n_steps = 10;
n_per_initstep = 600; 
N_T = 2000; 
...
list_cell = cell(1, n_steps);
evaluation_cell = cell(1, n_steps); 
%% t = 0
...
for t = 1:n_steps     
    ...
    %v_list = S_find_v_list_cv(weight, X, Y, Z, S_all, h_new_mat);
%     [s_list, v_list] = S_find_v_list_ce_value_all(weight, Z, S_all);
    [s_list, v_list] = S_find_v_list_ce_value_order_all(weight, Z, S_all);
    
    [select_list, sample_screen] = Pareto_select(X, Z, h_new_mat, weight, v_list, combo, n_per_step);
    list_cell{t} = select_list;
    %v_list = 1:10;

    v_index_list = [];
%     for i = 1:5
%         v_index_list = [v_index_list, index_find(v_list(i))];
%     end

    %v_count = S_find_v_list_cv_index_test(X, Z, h_new_mat);
    [v_count, train_list] = S_find_v_list_cv_index_measure_evaluation(X, Z, h_new_mat, select_list, n_per_step);
    v_position = find(select_list == v_count);

    evaluation_cell{t} = train_list;

    v_list = index_find(v_count);
    ...
    X_new = sample_screen((1 + (v_position - 1) * 200): (v_position * 200), :); 
